    ...
    public class Calculator
    {
        ...
        public static string InputBuffer
        {
            ...
            private set
            {
                ...
                {
                    if (_inputBuffer != string.Empty && value.Length >= _inputBuffer.Length)
{
        ...
        public static void EnterDot()
{
    if (isInputAllowed() && (!InputBuffer.Contains(decimalSeparator.ToString()) || doesTheInputContainResOrPrevNum || doesTheInputContainConstantValue))
    {
        ...
        {
            // Перевірка кількості нулів після коми
            int decimalIndex = InputBuffer.IndexOf(decimalSeparator);
            if (decimalIndex == -1 || InputBuffer.Length - decimalIndex <= 5)
        ...
        {
            if (InputBuffer.Length > 0 && !isBlocked && !doesTheInputContainConstantValue && !isUnaryOperation)
            {
        ...
        public static void Factorial()
        {
            ...
            {
                long num;
                if (Int64.TryParse(InputBuffer, out num))
                {
                    long res = num;
                    if (num > 0)
                    {
                    ...
                    {
                        InputBuffer = res.ToString("0.###e+0");
                    }
                    else
                    {
